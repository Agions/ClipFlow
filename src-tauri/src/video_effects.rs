...
fn filter_description(filter: &VideoFilter) -> String {
    match filter {
        ...
        VideoFilter::Contrast(v) => format!("对比度 {v:.1}"),
        VideoFilter::Brightness(v) => format!("亮度 {v:.1}"),
        VideoFilter::Saturation(v) => format!("饱和度 {v:.1}"),
        VideoFilter::Blur(r) => format!("模糊 {r:.1}px"),
        VideoFilter::Sharpen(s) => format!("锐化 {s:.1}"),
        VideoFilter::Vignette => "暗角 (Vignette)".to_string(),
        VideoFilter::Grain(g) => format!("胶片颗粒 {g:.2}"),
