...
const AlertDialogContent: React.FC<AlertDialogContentProps> = ({
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 z-50 flex items-center justify-center bg-bg-elevated/80 backdrop-blur-sm',
        className
      )}
      onClick={e => {
      ...
      {...props}
    >
      <div className="relative z-50 w-full max-w-lg rounded-lg bg-bg-elevated border border-border-default p-6 shadow-lg">
        {children}
      </div>
    </div>
  );
...
const AlertDialogDescription: React.FC<React.HTMLAttributes<HTMLParagraphElement>> = ({
  className,
  ...props
}) => <p className={cn('text-sm text-text-secondary mb-4', className)} {...props} />;
...
const AlertDialogCancel: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({
  ...
  <button
    className={cn(
      'inline-flex items-center justify-center rounded-md border border-border-strong bg-transparent px-4 py-2 text-sm font-medium text-text-primary hover:bg-bg-hover',
      className
    )}
    {...props}
  />
